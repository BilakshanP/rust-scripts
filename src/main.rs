...
#[allow(dead_code)]
fn keccak(rate_in_bits: u128, capacity: u128, input_bytes: &[u128], delimited_suffix: u128, output_byte_len: u128) -> Vec<u128> {
...
mod tests {
    ...
    fn load_64() {
        ...
        assert_eq!(super::load_64(&[1, 2, 3, 4, 5, 6, 7, 8]), 578437695752307201);
    
        assert_eq!(super::load_64(&[255, 255, 255, 255, 255, 255, 255, 255]), 18446744073709551615)
    }

    #[test]
    fn store_64() {
        assert_eq!(super::store_64(0), vec![0, 0, 0, 0, 0, 0, 0, 0]);
        assert_eq!(super::store_64(72340172838076673), vec![1, 1, 1, 1, 1, 1, 1, 1]);
        assert_eq!(super::store_64(578437695752307201), vec![1, 2, 3, 4, 5, 6, 7, 8]);
        assert_eq!(super::store_64(18446744073709551615), vec![255, 255, 255, 255, 255, 255, 255, 255]);
    }
    ...
    fn keccak_f1600_on_lanes() {
        assert_eq!(
            ...
            vec![
                ...
                vec![13624874521033984333, 12479658147685402012,  4879704952849025062, 13293599522548616907,  8500057116360352059],
                vec![10027350355371872343,  3500241080921619556,   140226327413610143, 10105770293752443592, 16929593379567477321]
            ]
        );
        ...
        assert_eq!(
            ...
            vec![
                ...
                vec![12256180026113095957,  7802670506913381763,  4066428769703078281, 18037914650129283290, 11984217004935637960],
                vec![ 1871358968484233706,  5803652345229422514, 17020757572326394724,  7947227104366760550, 17492745099396503234]
            ]
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                ...
                176,  60,  68, 181,  42, 167, 127,  14,  62,  40,  49, 107, 209, 182, 175, 236,   9,  81, 188,   8,
                 52, 146,   3, 204,  59,   2, 229,  29, 148, 218,  98, 248,   8, 156, 196, 242, 110, 157, 182, 149,
                  6,  23, 206, 158, 183, 172,  35,  85,  26, 222, 120, 252,  36, 110,   0,  36, 178, 218,  25, 176,
                  6,  62,  11,  41, 180, 209,  47, 235,  46,  65, 184, 227,  84, 182, 199,  44,  65, 170, 173,  49,
                228, 183,  68,  75, 169, 186, 229,  33, 157,   3,  92, 149, 142, 129, 220, 121,  67,  93,  49,  81,
                189, 196,  28, 228, 194,  64, 253, 228, 252, 160,  62, 124, 234,  97, 120,  54,  13,  53, 223,  13,
                 42, 243,  44, 243, 163,  11, 202, 146, 221, 204, 119, 197,   2, 103, 137, 163, 222, 169, 188, 218,
                229, 194, 199, 111,  89,  65,  15, 246,  86, 132, 161,  15,  22, 174,  15, 227, 212, 129,   8,   7
            ]
        )
    }
    ...
    #[test]
    fn keccak() {
        assert_eq!(super::keccak(1600,    0, &[  0,   0,   0,   0], 0x00,   8), vec![97,   96,  30, 159, 251,  73,  67,  91]);
        assert_eq!(super::keccak( 200, 1400, &[255, 255, 255, 255], 0xFF,   8), vec![242,  80,  39, 210,  73,   4,  71, 197]);

        assert_eq!(super::keccak( 800,  800, &[], 0xFF,  16), vec![ 42, 230, 159, 117,  24,   0, 170, 245, 107,  29,   6,  88, 108, 163, 181,  71]);
        assert_eq!(super::keccak(1600,    0, &[], 0xFF,  16), vec![165,   7,  17, 179,  63, 250,  21, 160, 144,  85, 162, 243, 141,  75,  74,  46]);

        assert_eq!(
            super::keccak(1472,  128, &[], 0x00, 128),
            vec![
                  1, 230, 255, 236,  50, 175,   7,  40, 116, 100, 103, 217, 240, 151, 122, 191,
                145, 126,  44, 169, 211,  42,  83,  76,  25, 204,  14, 174,  20, 113,  82, 182,
                  1, 117,  55, 187, 216,  12, 120, 190, 216,  12, 252,  31, 104, 222, 239, 198,
                208, 254, 111, 116, 102,  29, 108, 172,  26, 159, 141,  33,  79,  37,  35, 104,
                217, 116,  64,  71, 250, 139,  85,   3, 230, 110, 229, 238, 193, 223, 134,  41,
                 86,  72, 138, 115,  90, 153,  69, 211, 196, 116,  67, 211, 200, 208,  62, 238,
                180,  18, 211, 230,   5, 103, 236, 110,  54, 251, 232,  21, 248,  62, 204, 110,
                185, 118, 203, 152,  92, 176,  49, 147,  26, 225, 158,  51, 180, 190, 148, 176
            ]
        )
    }
}
